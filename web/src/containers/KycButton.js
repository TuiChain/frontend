...
const KycButton = props => {
  ...
  const handleStripeResponse = (event) => {
    ...
    if (event.origin === 'https://verify.stripe.com' && intentId !== undefined){
  ...
  return (
    <div>
      <TransitionsModal open={open} src={url} handleClose={() => setOpen(false)}>
        <iframe allow="camera; microphone" className={classes.iframe} src={url}/>
      </TransitionsModal>
      <Button
        onClick={handleButtonClick}
        disabled={verificationStatus !== undefined}
        style={{color:theme.palette.secondary.contrastText, backgroundColor: statusColors[verificationStatus] ? statusColors[verificationStatus] : theme.palette.primary.main}}
      >
        {renderText()}
      </Button>
    </div>
