...
const LoanRequests = (props) => {
  ...
  useEffect(() => {
    async function fetchRequests() {
      const data = await LoansService.getPendingLoanRequests();
      console.log(data);
      setRequests(data);
      setLoading(false);
    }
  ...
  const acceptRequest = async (id) => {
    console.log("ID:", id);
    const valid = await LoansService.validateLoanRequest(id);
  ...
  const rejectRequest = async (id) => {
    ...
    const valid = await LoansService.closeLoanRequest(id);
